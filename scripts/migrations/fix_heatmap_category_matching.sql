-- Migration: fix_heatmap_category_matching
-- Description: Fix category matching to use display_name prefix instead of internal name
-- The modifier_group contains display names like "Car Insurance" but the internal
-- category name is "car_insurance___canonical" and display_name is "Car Insurance - Canonical"
-- We need to match by display_name prefix (case-insensitive)
--
-- Run this in Supabase SQL Editor

-- Fix get_modifier_group_heatmap
...
BEGIN
    ...
    SELECT name, display_name INTO v_row_cat_name, v_row_cat_display
    FROM categories
    ...
    SELECT name, display_name INTO v_col_cat_name, v_col_cat_display
    FROM categories
    WHERE market_id = p_market_id
      AND (
    ...
    IF v_row_cat_name IS NULL OR v_col_cat_name IS NULL THEN
        RETURN json_build_object(
            ...
            'row_category', COALESCE(v_row_cat_name, p_row_category),
            'row_category_display', COALESCE(v_row_cat_display, p_row_category),
            'col_category', COALESCE(v_col_cat_name, p_col_category),
            'col_category_display', COALESCE(v_col_cat_display, p_col_category),
            'row_values', '[]'::json,
            'col_values', '[]'::json,
            'cells', '[]'::json,
            'max_volume_uncaptured', 0,
    ...
    -- Get row category values for filtered keywords (using resolved category name)
    row_tags AS (
        SELECT kt.keyword_id, kt.value as row_value
        FROM keyword_tags kt
        JOIN categories c ON kt.category_id = c.id
        WHERE c.market_id = p_market_id
          AND c.name = v_row_cat_name
          AND kt.keyword_id IN (SELECT id FROM filtered_keywords)
    ),
    -- Get col category values for filtered keywords (using resolved category name)
    col_tags AS (
        SELECT kt.keyword_id, kt.value as col_value
        FROM keyword_tags kt
        JOIN categories c ON kt.category_id = c.id
        WHERE c.market_id = p_market_id
          AND c.name = v_col_cat_name
          AND kt.keyword_id IN (SELECT id FROM filtered_keywords)
    ),
    ...
    SELECT json_build_object(
        ...
        'row_category_display', v_row_cat_display,
        'col_category', v_col_cat_name,
        'col_category_display', v_col_cat_display,
...
BEGIN
    ...
    -- Find row category by matching display_name prefix (case-insensitive)
    -- This handles cases where modifier_group contains "Car Insurance" but category display_name is "Car Insurance - Canonical"
        ...
        SELECT
            ...
            COALESCE(
                (SELECT bd.domain_type FROM brand_domains bd
                 ...
                 LIMIT 1),
                (SELECT mdt.name FROM market_domain_types mdt
                 WHERE mdt.market_id = p_market_id AND mdt.name = '3rd Party'
                 LIMIT 1),
                '3rd Party'
